// --- 1. НАСТРОЙКИ ---
var _speed = 12;        // Скорость бега (чуть прибавил, 2 — совсем медленно)
var _jump_power = -20; // Высота прыжка
...
if (!_is_on_floor) {
    ...
    if (_move_x != 0) {
        sprite_index = Sprite_walk1; // Идем
        image_xscale = _move_x;      // Теперь смотрит туда, куда идет! (Убрали минус)
    } else {
        sprite_index = Sprite2; // Стоим
    }
}